...
using System;
using System.IO;
using StegaMail.FJCore.Decoder;

namespace StegaMail.FJCore.Encoder
...

{
    ...
    public delegate void delegateEditDCT(int[] dct);

    public class JpegEncoder
    {
        public static byte zm = 0;
        JpegEncodeProgressChangedArgs _progress;
        public event delegateEditDCT editDCT;
        ...
        internal void CompressTo(Stream outStream)
        {
                    ...
                    for (comp = 0; comp < _input.Image.ComponentCount; comp++)
                    {
                        ...
                        for (i = 0; i < _input.VsampFactor[comp]; i++)
                        {
                            for (j = 0; j < _input.HsampFactor[comp]; j++)
                            {
                                ...
                                //TODO:UKRYWANIE WARTOŚCI
                                if(index < Plik.GetSize() && value % 3 == 0) {
                                    if(index < 0) {
                                        ProsteKodowanie.CodeSize(ref dctArray3, index + 4);
                                    }
                                    else {
                                        ProsteKodowanie.CodeBlock(ref dctArray3, index);
                                    }
                                    index++;
                                }
                                value++;
            ...
            if(value / 3 - 4 < Plik.GetSize()) {
                throw new Exception(Error.SMALL);
            }
