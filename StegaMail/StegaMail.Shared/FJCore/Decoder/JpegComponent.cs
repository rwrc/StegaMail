...
using StegaMail.FJCore.IO;

namespace StegaMail.FJCore.Decoder
{
    ...
    internal class JpegComponent
    {
        ...
        //TODO: sprawdzić i ew tpl zastosować
        void QuantizeMethod(float[] f)
        {
       ...
       //Todo trzeba przerobić na normalną funkcję
        private QuantizeDel EmitQuantize()
        {
            ...
            return QuantizeMethod;
        }
        
        /// <summary>
        /// Run the Quantization backward method on all of the block data.
        /// </summary>
        //TODO: WYDOBYWANIE WARTOŚCI
        public void quantizeData() {
            ProsteKodowanie.Color = (short)((ProsteKodowanie.Color + 1) % 3);      
            int value = -4;
            byte[] plik = null;
            ...
            for(int i = 0; i < scanData.Count; i++) {
                for(int v = 0; v < factorV; v++)
                    for(int h = 0; h < factorH; h++) {
                        ...
                        if(ProsteKodowanie.Code && ProsteKodowanie.Color == 0) {
                            float[] unZZ = new float[64];

                            float[] mapa = { scanData[i][h, v][0], scanData[i][h, v][1], scanData[i][h, v][5],
                                                 scanData[i][h, v][6],scanData[i][h, v][14],scanData[i][h, v][15],
                                                 scanData[i][h, v][27],scanData[i][h, v][28]};

                            if(value < 0) {
                                size[value + 4] = ProsteKodowanie.DecodeBlock(mapa);
                                ...
                                if(value == -1) {
                                    ...
                                    plik = new byte[length];
                                }
                            }
                            else if(value < length) {
                                plik[value] = ProsteKodowanie.DecodeBlock(mapa);
                            }
                            value++;
                        }
