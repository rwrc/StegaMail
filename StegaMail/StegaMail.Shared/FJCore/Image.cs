...
/* zmodyfikowano
        ...
        public Bitmap ToBitmap()
        {
          ...
          //  {
                ...
                //BitmapData[] ptrBitmap = bmData.Scan0;
                byte bitsPerPixel = 24;// GetBitsPerPixel(bmData.PixelFormat);
                ...
                for (int y = 0; y < _height; y++)
                {
                    ...
                    {
                        data[0+pos] = (byte)_raster[0][x, y];
                        data[1 + pos] = (byte)_raster[1][x, y];
                        data[2 + pos] = (byte)_raster[2][x, y];

                        ColorConverter(ref data[0], ref data[1], ref data[2]);

                        // Swap RGB --> BGR
                        byte R = data[0 + pos];
                        data[0 + pos] = data[2 + pos];
                        data[2 + pos] = R;

                        data[3 + pos] = 255; 
                        pos += 4;     // advance to the next pixel
                        i++;                // "
                    }
                }
        //    }
